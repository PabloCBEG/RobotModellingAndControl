...
c1a = (-y + sqrt(y^2 - ((z - 1)^2 + y^2)*(1 - (z - 1)^2)))/((z - 1)^2 +y^2);
c1b = (-y - sqrt(y^2 - ((z - 1)^2 + y^2)*(1 - (z - 1)^2)))/((z - 1)^2 +y^2);
...
q3(4) = (-x - L2*sin(q2(4)))/cos(q2(4)) - L3A;
    ...
    for i=1:4
        if(q2(i) == 0)
            ...
            if(abs(c1) > 1) fprintf('Error en el calculo de cos(q1): solucion no valida en indice %i',i); end
            s1 = sqrt(1 - c1^2);% or -sqrt
            q1(i) = atan2(s1,c1);
        else
            c1(i) = (L3A + q3(i) + x*cos(q2(i)) + ((y*sin(q2(i)))/(z - L0))*...
                (L3B + L1))/((L0 - z)*sin(q2(i)) - (y^2*sin(q2(i)))/(z - L0));
            if(abs(c1(i)) > 1) fprintf('Error en el calculo de cos(q1): solucion no valida en indice %i',i); end
